...
use super::schemas::{
    ...
    UpdatePreferencesRequestBody, UpdateUsernameRequestBody, User, UserDataResponse,
    UserOpenPositionsStats, UserPnlStats, UserPosition, UserTradesResponseBody, UserVolumeStats,
};
...
pub async fn get_profile_user_data(
    proxy_wallet_address: &str,
    proxy: Option<&Proxy>,
) -> Result<UserDataResponse, CustomError> {
    let url = format!(
        "https://polymarket.com/api/profile/userData?address={}",
    ...
    let response = send_http_request_with_retries::<UserDataResponse>(
        &request_params,
        None,
        proxy,
        None,
    ...
    response
        .body
        .ok_or_else(|| CustomError::PolymarketApi("empty profile user data response".to_string()))
}
