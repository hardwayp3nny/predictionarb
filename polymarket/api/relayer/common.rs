...
use alloy_primitives::{bytes, Address, FixedBytes, U256};
use alloy_signer::Signer;
use alloy_sol_types::{self, sol, SolCall};
use reqwest::Proxy;
...
use super::{
    constants::{
        ...
        NEG_RISK_ADAPTER_CONTRACT_ADDRESS, UCHILD_ERC20_PROXY_CONTRACT_ADDRESS,
    },
...
pub async fn split_position<S: Signer + Send + Sync>(
    signer: Arc<S>,
    amp_cookie: &mut AmpCookie,
    polymarket_nonce: &str,
    polymarket_session: &str,
    ...
    let condition_id_bytes: [u8; 32] =
        const_hex::decode(condition_id.strip_prefix("0x").unwrap_or(condition_id))
            .map_err(|e| CustomError::Decode(e.to_string()))?
            .try_into()
            .map_err(|_| CustomError::Decode("Invalid condition_id length".to_string()))?;

    let calldata = splitPositionCall {
        collateralToken: UCHILD_ERC20_PROXY_CONTRACT_ADDRESS,
        parentCollectionId: FixedBytes::ZERO,
        conditionId: FixedBytes(condition_id_bytes),
        partition: vec![U256::from(1), U256::from(2)],
        amount,
    }
    .abi_encode();

    let target_contract = if neg_risk {
        NEG_RISK_ADAPTER_CONTRACT_ADDRESS
    } else {
    ...
    .await
    .map_err(|err| CustomError::PolymarketApi(err.to_string()))?;

    let data_hex = const_hex::encode_prefixed(calldata);
...
pub async fn merge_position<S: Signer + Send + Sync>(
    signer: Arc<S>,
    amp_cookie: &mut AmpCookie,
    polymarket_nonce: &str,
    polymarket_session: &str,
